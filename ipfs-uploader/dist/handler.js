...
function validateMime(form) {
    const ct = (form.contentType || form.file?.contentType || '').toLowerCase();
    const ok = ct === 'image/png' || ct === 'application/json';
    if (!ok)
        throw { code: 'VALIDATION_FAILED', message: 'content_type must be one of [image/png, application/json]', status: 400 };
    return ct;
}
export const handler = async (event) => {
    try {
        ...
        if (!form.file)
            throw { code: 'VALIDATION_FAILED', message: 'file is required', status: 400 };
