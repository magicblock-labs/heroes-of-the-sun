...
namespace View
{
    ...
    {
        [Inject] private PathfindingModel _model;
        ...
        void Update()
        {
            ...
            if (Input.GetMouseButtonUp(0) && Time.time - _mouseDownTime < .5f)
            {
                ...
                _path = _model.FindPath(_location, tile.Location);
                MoveToNextPoint();
            }
            ...
            if (_currentTarget.HasValue)
            {
                ...
                _line.SetPositions(_path
                    .Select(v => ConfigModel.GetWorldCellPosition(v.x, v.y) + Vector3.up * (_model.GetY(v) + 2f))
                    .ToArray());
            }
        }
        ...
        private void MoveToNextPoint()
        {
            if (_path.Count > 0)
            {
                _currentTarget = ConfigModel.GetWorldCellPosition(_path[0].x, _path[0].y) +
                                 Vector3.up * (_model.GetY(_location) + 1f);
            }
            else
            {
                _currentTarget = null;
                transform.position = ConfigModel.GetWorldCellPosition(_location.x, _location.y) +
                                     Vector3.up * (_model.GetY(_location) + 1f);
