...
using Smartobjectlocation;
using Smartobjectlocation.Program;
using Smartobjectlocation.Errors;
using Smartobjectlocation.Accounts;
using Smartobjectlocation.Types;

namespace Smartobjectlocation
{
    namespace Accounts
    {
        ...
        public partial class SmartObjectLocation
        {
            public static ulong ACCOUNT_DISCRIMINATOR => 1029500287737452147UL;
            public static ReadOnlySpan<byte> ACCOUNT_DISCRIMINATOR_BYTES => new byte[]{115, 214, 192, 162, 14, 133, 73, 14};
            public static string ACCOUNT_DISCRIMINATOR_B58 => "LNnNMme2YdK";
            ...
            public static SmartObjectLocation Deserialize(ReadOnlySpan<byte> _data)
            {
                int offset = 0;
                ulong accountHashValue = _data.GetU64(offset);
                offset += 8;
                ...
                SmartObjectLocation result = new SmartObjectLocation();
                result.X = _data.GetS32(offset);
                offset += 4;
                result.Y = _data.GetS32(offset);
                offset += 4;
                result.Entity = _data.GetPubKey(offset);
                offset += 32;
    ...
    namespace Errors
    {
        public enum SmartobjectlocationErrorKind : uint
        {
        }
    }
    ...
    public partial class SmartobjectlocationClient : TransactionalBaseClient<SmartobjectlocationErrorKind>
    {
        public SmartobjectlocationClient(IRpcClient rpcClient, IStreamingRpcClient streamingRpcClient, PublicKey programId = null) : base(rpcClient, streamingRpcClient, programId ?? new PublicKey(SmartobjectlocationProgram.ID))
        {
        }

        public async Task<Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<Entity>>> GetEntitysAsync(string programAddress = SmartobjectlocationProgram.ID, Commitment commitment = Commitment.Confirmed)
        {
        ...
        public async Task<Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<SmartObjectLocation>>> GetSmartObjectLocationsAsync(string programAddress = SmartobjectlocationProgram.ID, Commitment commitment = Commitment.Confirmed)
        {
            var list = new List<Solana.Unity.Rpc.Models.MemCmp>{new Solana.Unity.Rpc.Models.MemCmp{Bytes = SmartObjectLocation.ACCOUNT_DISCRIMINATOR_B58, Offset = 0}};
            var res = await RpcClient.GetProgramAccountsAsync(programAddress, commitment, memCmpList: list);
            if (!res.WasSuccessful || !(res.Result?.Count > 0))
                return new Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<SmartObjectLocation>>(res);
            List<SmartObjectLocation> resultingAccounts = new List<SmartObjectLocation>(res.Result.Count);
            resultingAccounts.AddRange(res.Result.Select(result => SmartObjectLocation.Deserialize(Convert.FromBase64String(result.Account.Data[0]))));
            return new Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<SmartObjectLocation>>(res, resultingAccounts);
        }
        ...
        public async Task<Solana.Unity.Programs.Models.AccountResultWrapper<SmartObjectLocation>> GetSmartObjectLocationAsync(string accountAddress, Commitment commitment = Commitment.Finalized)
        {
            var res = await RpcClient.GetAccountInfoAsync(accountAddress, commitment);
            if (!res.WasSuccessful)
                return new Solana.Unity.Programs.Models.AccountResultWrapper<SmartObjectLocation>(res);
            var resultingAccount = SmartObjectLocation.Deserialize(Convert.FromBase64String(res.Result.Value.Data[0]));
            return new Solana.Unity.Programs.Models.AccountResultWrapper<SmartObjectLocation>(res, resultingAccount);
        }
        ...
        public async Task<SubscriptionState> SubscribeSmartObjectLocationAsync(string accountAddress, Action<SubscriptionState, Solana.Unity.Rpc.Messages.ResponseValue<Solana.Unity.Rpc.Models.AccountInfo>, SmartObjectLocation> callback, Commitment commitment = Commitment.Finalized)
        {
            SubscriptionState res = await StreamingRpcClient.SubscribeAccountInfoAsync(accountAddress, (s, e) =>
            {
                SmartObjectLocation parsingResult = null;
                if (e.Value?.Data?.Count > 0)
                    parsingResult = SmartObjectLocation.Deserialize(Convert.FromBase64String(e.Value.Data[0]));
                callback(s, e, parsingResult);
            }, commitment);
            return res;
        }

        protected override Dictionary<uint, ProgramError<SmartobjectlocationErrorKind>> BuildErrorsDictionary()
        {
            return new Dictionary<uint, ProgramError<SmartobjectlocationErrorKind>>{};
        }
    }
    ...
    namespace Program
    {
        ...
        public static class SmartobjectlocationProgram
        {
            public const string ID = "5ewDDvpaTkYvoE7ZJJ9cDmZuqvGQt65hsZSJ9w73Fzr1";
