...
namespace Lootdistribution
{
    namespace Accounts
    {
        ...
        public partial class LootDistribution
        {
            ...
            public static LootDistribution Deserialize(ReadOnlySpan<byte> _data)
            {
                int offset = 0;
                ulong accountHashValue = _data.GetU64(offset);
                offset += 8;
                ...
                offset += 4;
                result.Loots = new LootLocation[resultLootsLength];
                for (uint resultLootsIdx = 0; resultLootsIdx < resultLootsLength; resultLootsIdx++)
                {
                    offset += LootLocation.Deserialize(_data, offset, out var resultLootsresultLootsIdx);
                    result.Loots[resultLootsIdx] = resultLootsresultLootsIdx;
                }
    ...
    namespace Types
    {
        ...
        public partial class LootLocation
        {
            public int X { get; set; }

            public int Y { get; set; }

            public sbyte LootType { get; set; }

            public int Serialize(byte[] _data, int initialOffset)
            {
                int offset = initialOffset;
                _data.WriteS32(X, offset);
                offset += 4;
                _data.WriteS32(Y, offset);
                offset += 4;
                _data.WriteS8(LootType, offset);
                offset += 1;
                return offset - initialOffset;
            }

            public static int Deserialize(ReadOnlySpan<byte> _data, int initialOffset, out LootLocation result)
            {
                int offset = initialOffset;
                result = new LootLocation();
                result.X = _data.GetS32(offset);
                offset += 4;
                result.Y = _data.GetS32(offset);
                offset += 4;
                result.LootType = _data.GetS8(offset);
                offset += 1;
                return offset - initialOffset;
            }
        }
    ...
    {
        public LootdistributionClient(IRpcClient rpcClient, IStreamingRpcClient streamingRpcClient, PublicKey programId) : base(rpcClient, streamingRpcClient, programId)
        {
        }
        ...
        public async Task<Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<LootDistribution>>> GetLootDistributionsAsync(string programAddress, Commitment commitment = Commitment.Confirmed)
        {
